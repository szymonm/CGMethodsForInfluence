...
trait LDAGApproximation extends Cache with LazyLogging {
  ...
  def computeLDAG(forNode: Int, withInfluences: Boolean = false): (immutable.Map[Int, Int], Option[Map[Int, Double]]) = {
...
//          )
  ...
  def ldagPredecessorsTraverse(ldagHead: Int, traverseStart: Int)(nodeVisitor: (g.NodeT, Double) => Boolean) {
    ...
    while (!toVisit.isEmpty) {
      ...
      if (nodeVisitor(cur, weight)) {
        cur.incoming.foreach {
          edge =>
            if (ldagNodes(ldagHead).getOrElse(edge.from.value, 0) > curLDAGPosition) {
