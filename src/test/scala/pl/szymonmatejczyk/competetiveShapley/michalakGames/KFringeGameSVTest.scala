...
class KFringeGameSVTest extends FlatSpec with ShouldMatchers {
  ...
  "KFringeGameSV" should "compute SV in simplest case" in {
    n.computeKFringeSV(2) shouldEqual Map(1 -> 1.0, 2 -> 1.0)
    n.computeKFringeSV(1) shouldEqual Map(1 -> 1.5, 2 -> 0.5)
  }
  ...
  it should "compute SV for cycle3" in {
    ...
    n1.computeKFringeSV(2) shouldEqual Map(1 -> 1.0, 2 -> 1.0, 3 -> 1.0)
  }
}